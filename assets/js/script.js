...
if($('.datanew').length>0){$('.datanew').DataTable({"bFilter":true,"sDom":'fBtlpi','pagingType':'numbers',"ordering":true,"language":{search:' ',sLengthMenu:'_MENU_',searchPlaceholder:"Rechercher...",info:"_START_ - _END_ sur _TOTAL_ éléments",zeroRecords:"Aucun résultat trouvé",emptyTable:"Aucune donnée disponible",infoEmpty:"0 - 0 sur 0 élément",infoFiltered:"(filtré sur _MAX_ éléments au total)",paginate:{first:"Premier",last:"Dernier",next:"Suivant",previous:"Précédent"},},initComplete:(settings,json)=>{$('.dataTables_filter').appendTo('#tableSearch');$('.dataTables_filter').appendTo('.search-input');},});}
